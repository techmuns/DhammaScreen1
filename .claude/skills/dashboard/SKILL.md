---
name: dashboard
description: Standards, patterns, and best practices for building Munshot-embedded dashboards. Use when creating, modifying, or reviewing any dashboard widget, layout, KPI, chart, table, or filter; when wiring up the Munshot Dashboard SDK; when picking datasources from the registry (web_search, web_reader, news_search, document_search, muns_chat, agent_run); or whenever the user asks about dashboard UI shell, 3-zone layout, WidgetCard structure, design tokens, loading/empty/error states, source trails, or dashboard variants (screener, sector, heatmap, brand discounts, document intelligence, portfolio).
---
